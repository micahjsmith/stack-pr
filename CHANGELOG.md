...
* In a land (`l`) step in an `autoland` plan, a specific pull request can be
  ...
  land the next PR in the stack. This change allows an autoland plan to be
  durable across an entire stack, and can be reused if PRs are landed: a step
  whose PR has already merged is skipped, as are any `w`/`c` steps before it,
  and `autoland` fails before doing anything if the next PR in the stack isn't
  the one the next live `l` step names. A bare `l` still lands the next PR in
  the stack, and the two forms can be mixed. Plans generated by `-i` now name
  their PRs (`l 123`) so that a saved plan is reusable by default. (<insert PR
  number>).
* `autoland` gained `--plan-file PATH`: load the landing plan from a file
  ...
  and can't be combined with `--count` or `--resume`. (<insert PR number>).
...
* **Breaking:** `submit` no longer copies the commit title into the PR
  ...
  the next `submit` (unless `--keep-body` is set). (<insert PR number>).
